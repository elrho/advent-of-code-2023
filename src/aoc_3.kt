...
fun findSymbols(input: String, listNumber: Int): MutableList<List<Int>> {
    val symbolCoOrds = mutableListOf<List<Int>>()
    val symbols = listOf('!', '@', '#', '$', '%', '&', '*', '+', '/', '=')
    for (symbol in symbols) {
        if (input.contains(symbol)) {
            val iList = input.map { it }
            var indexes = iList.indexesOf(symbol).map { listOf(listNumber, it) }
            symbolCoOrds += indexes
        }
    }
...
fun findParts(input: String): List<Int> {
    val inputList = input.split("\n")
    val symbols = mutableListOf(mutableListOf(listOf<Int>()))
    inputList.forEach {
        var symbolsCoords = findSymbols(it, inputList.indexOf(it))
        symbols += symbolsCoords
    }
    val parts = mutableListOf<Int>()
    symbols.forEach {
        it.forEach { coord ->
            if (coord.isNotEmpty()) {
                if (inputList[coord.first()][coord.last() - 1].isDigit()) {
                    parts += findLeftDigit(inputList[coord.first()], (coord.last() - 1), mutableListOf())
                }
                if (inputList[coord.first()][coord.last() + 1].isDigit()) {
                    parts += findRightDigit(inputList[coord.first()], (coord.last() + 1), mutableListOf())
                }
            }
        }
    }
...
fun findLeftDigit(input: String, index: Int, partNumbers: MutableList<Int>): MutableList<Int> {
    if (input[index] == '.') return partNumbers
    else {
        partNumbers += input[index].toString().toInt()
        findLeftDigit(input, index - 1, partNumbers)
    }
    return mutableListOf(partNumbers.asReversed().joinToString("").toInt())
}

fun findRightDigit(input: String, index: Int, partNumbers: MutableList<Int>): MutableList<Int> {
    if (input[index] == '.') return partNumbers
    else {
        partNumbers += input[index].toString().toInt()
        findRightDigit(input, index + 1, partNumbers)
    }
    return mutableListOf(partNumbers.joinToString("").toInt())
}

fun findAboveLeft(input: List<String>, indexes: List<Int>, partNumbers: MutableList<Int>): MutableList<Int> {
    val aboveLeft = input[indexes.first()][indexes.last()]
    if (aboveLeft == '.' || aboveLeft == null) return partNumbers
    else {
        partNumbers += aboveLeft.toString().toInt()
        findAboveLeft(input, listOf(indexes.first(), (indexes.last() - 1)), partNumbers)
    }
    return mutableListOf(partNumbers.joinToString("").toInt())
}

fun findAbove(input: List<String>, indexes: List<Int>, partNumbers: MutableList<Int>): MutableList<Int> {
    val above = input[indexes.first()][indexes.last()]
    if (above == '.' || above == null) return partNumbers
    else if (above.isDigit() && input[indexes.first() - 1][indexes.last() - 1].isDigit() && input[indexes.first() - 1][indexes.last() + 1].isDigit()) {
        var tempParts = listOf(input[indexes.first() - 1][indexes.last() - 1], above, input[indexes.first() - 1][indexes.last() + 1])
        tempParts.forEach { partNumbers.add(it.toString().toInt()) }
    }
    else if (above.isDigit() && input[indexes.first() - 1][indexes.last() - 1] == '.') {
        partNumbers+=findRightDigit(input[indexes.first()], indexes.last(), mutableListOf())
    }
    else {
        partNumbers += above.toString().toInt()
        findAboveLeft(input, listOf(indexes.first(), (indexes.last() + 1)), partNumbers)
    }
    return mutableListOf(partNumbers.joinToString("").toInt())
}
...
//const val input = """.......12.......935............184.720...243........589.652..........435..........483.............6...........................904...........
