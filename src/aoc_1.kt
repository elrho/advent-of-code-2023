...
fun numbersForEachLine(line: String): Int {
    val writtenNumbers = mutableMapOf<Int, Int>()
    for (i in numbers.keys) {
        if (line.contains(i)) {
            writtenNumbers += line.indexOf(i) to numbers[i]!!
            writtenNumbers += line.lastIndexOf(i) to numbers[i]!!
        }
    }
    val nums = line.filter { it.isDigit() }.map { it }
    for (i in nums) {
        writtenNumbers += line.indexOf(i) to i.toString().toInt()
        writtenNumbers += line.lastIndexOf(i) to i.toString().toInt()
    }
    val orderedNumbers = writtenNumbers.toSortedMap()
    return if (orderedNumbers.size == 1) "${orderedNumbers.values.first()}${orderedNumbers.values.first()}".toInt()
    else "${orderedNumbers.values.first()}${orderedNumbers.values.last()}".toInt()
}

fun addResultsFromLine(input: String): Int {
    val inputList = input.split("\n")
    val numbers = inputList.map { numbersForEachLine(it) }
    return numbers.sum()
}

fun main() {
    println(addResultsFromLine(input))
}
