...
fun possibleGame(line: String, available: Map<String, Int>): Boolean {
    val set = line.replaceBefore(":", "").replace(": ", "").split("; ").map { it.split(", ") }
    val setMaps = set.map { it.associate { s -> s.substringAfter(" ") to s.substringBefore(" ").toInt() } }
    var count = 0
    var setsWon = 0
    val colours = listOf("blue", "green", "red")
    for (i in setMaps) {
        for (colour in colours) {
            if (i[colour] == null)
                count++
            else if (i[colour]!! <= available[colour]!!) {
                count++
            }
            if (count == 3) {
                setsWon++
    ...
    return setsWon == setMaps.size
}

fun minimumCubes(line: String): Int {
    val set = line.replaceBefore(":", "").replace(": ", "").split("; ").map { it.split(", ") }
    val setMaps = set.map { it.associate { s -> s.substringAfter(" ") to s.substringBefore(" ").toInt() } }
    val colours = listOf("blue", "green", "red")
    val minCubes = mutableMapOf("blue" to 0, "green" to 0, "red" to 0)
    setMaps.forEach {
        for (colour in colours) {
            if (it[colour] == null || it[colour]!! <= minCubes[colour]!!) continue
            else minCubes[colour] = it[colour]!!
        }
    }
    return minCubes.values.fold(1) {sum, element -> sum * element}
}

fun addNumbersOfPossibleGames(input: String): Int {
    val linesList = input.split("\n")
    val possibleGameNos = mutableListOf<Int>()
    linesList.forEach {
        var gameNo = it.substringAfter(" ").substringBefore(":").toInt()
        if (possibleGame(it, mapOf("blue" to 14, "green" to 13, "red" to 12))) possibleGameNos+=gameNo
        gameNo = 0
    }
    return possibleGameNos.sum()
}

fun productOfMinCubes(input: String): Int {
    val linesList = input.split("\n")
    return linesList.map { minimumCubes(it) }.fold(0) {sum, element -> sum+element}
}


fun main() {
    val input = """Game 1: 1 green, 6 red, 4 blue; 2 blue, 6 green, 7 red; 3 red, 4 blue, 6 green; 3 green; 3 blue, 2 green, 1 red
...
Game 100: 3 blue, 6 red, 9 green; 4 red, 3 green; 4 green, 16 red, 1 blue; 14 blue, 1 green"""
    println(productOfMinCubes(input))
}
